...
def uniform_search(filename):
    ...
    counter =0
    l = {}
    l.update({container[0]:container[0].state.estimated_distance})
    while len(container) > 0:
        key_min = min(l.keys(), key=(lambda k: l[k]))
        node = key_min
        del l[key_min]
        ...
        for next_node in successor:
            ...
            if map_state not in visited:
                ...
                if not deadlock_checker(next_node.state):
                    ...
                    l.update({next_node:counter+next_node.state.estimated_distance})
        i += 1
